# Transmitter Implementation Guide (`lossless_tx`)

This guide outlines how to configure, code, and compile the **Transmitter** (`lossless_tx`) project using the ESP-IDF framework under PlatformIO.
...
### `platformio.ini`
Replace the contents of `lossless_tx/platformio.ini` to use the ESP-IDF framework instead of Arduino, matching the seeed_xiao_esp32s3 board setup:
...
### Root `CMakeLists.txt`
Create a `CMakeLists.txt` in the `lossless_tx` root directory:
```cmake
...
Create an `idf_component.yml` in the `lossless_tx` root directory to declare the dependency on the Espressif UAC device component:
```yaml
dependencies:
  espressif/usb_device_uac: ">=0.0.1"
```

### Source Build Script `src/CMakeLists.txt`
Create `src/CMakeLists.txt` to register the application source files and link them with the TinyUSB/UAC libraries:
```cmake
FILE(GLOB_RECURSE app_sources ${CMAKE_SOURCE_DIR}/src/*.*)

idf_component_register(SRCS ${app_sources}
                       PRIV_REQUIRES espressif__usb_device_uac)
...
target_include_directories(${tusb_lib} PUBLIC "${uac_dir}/tusb" "${uac_dir}/tusb_uac")
```

### Source Component configuration `src/idf_component.yml`
Create `src/idf_component.yml` to request version matching for `usb_device_uac`:
```yaml
dependencies:
  espressif/usb_device_uac: ">=0.0.1"
```
...
## 2. Configuration Options (`sdkconfig.defaults`)

Create a `sdkconfig.defaults` file in the root of `lossless_tx` to define the configuration parameters for the USB Audio Class (UAC) stack. These match the latest updates in `usb_dac_espidf` (stereo configuration, increased new-play interval to prevent startup audio drops):
...
CONFIG_UAC_BIT_RESOLUTION=16
CONFIG_UAC_BYTES_PER_SAMPLE=2
CONFIG_UAC_SPK_INTERVAL_MS=10
CONFIG_UAC_MIC_INTERVAL_MS=10
CONFIG_UAC_SPK_NEW_PLAY_INTERVAL=1000
CONFIG_UAC_TINYUSB_TASK_PRIORITY=5
CONFIG_UAC_TINYUSB_TASK_CORE=-1
CONFIG_UAC_SPK_TASK_PRIORITY=5
CONFIG_UAC_SPK_TASK_CORE=-1
```

---

## 3. TinyUSB Header & Descriptor Setup

Copy `usb_descriptors.c` and `tusb_config.h` from `usb_dac_espidf` to `lossless_tx/src/` and `lossless_tx/include/` respectively. Ensure that `usb_descriptors.c` contains the updated USB PID of `0x8002` to force host operating systems to clear cached descriptors and recognize it as a stereo device.

---

## 4. Main Application Code (`src/main.c`)

Create `lossless_tx/src/main.c` with the following implementation. Key improvements include:
- **Sequential Sub-packet Transmission**: Spreads out ESP-NOW sub-packet transmission (~1 packet per 1.25ms) rather than bursting 8 packets at once, preventing ESP-NOW internal queue overflow.
- **Auto-pairing & Handshake**: Processes beacons from the receiver, registers the receiver peer with a range-optimized PHY rate (802.11g 24 Mbps), and replies with broadcast confirmation (`TX_CONF`) packets so the receiver can auto-register the transmitter.
- **Blink-Status LED Controller Task**: Distinguishes states visually: slow blink when searching, rapid blink when streaming audio, solid ON when paired but idle, and double-flash warnings on high rates of ESP-NOW transmit failure.
- **Unicast Connection Health Checks**: Sends a periodic unicast test packet to the paired receiver every 1 second to monitor connection health.
